 ...
 *
 * Ora allows `agent/list_models` 60 seconds, and a probe has to spawn a CLI and complete an ACP
 * handshake inside that. The budget covers the conversation rather than each request, so a slow
 * `initialize` cannot be followed by an equally slow `session/new` and overrun the host's own
 * deadline — which would fail discovery with a timeout naming the host instead of the CLI.
 */
const PROBE_BUDGET_MS = 40_000;

/** One probe request still waiting for the CLI's answer, keyed by its JSON-RPC id. */
...
/**
 * Speaks ACP to one short-lived CLI process the plugin questions and then discards.
 *
 * This is deliberately not the bridge in `opencode-client.ts`. That one is a pipe: Ora owns both
 * ends of the conversation and this plugin never injects a request of its own into it. A probe is
 * the opposite — this plugin is the ACP *client*, asking a question Ora knows nothing about — so
 * it needs its own request/response correlation and, above all, its own process. A request
 * injected into the connection Ora is reading would return its answer down Ora's pipe, and the
 * capabilities Ora declares in its own `initialize` are what decide whether the agent reports a
 * model selector at all.
 *
 * Nothing here is OpenCode-specific: the caller supplies an already-spawned host-owned process and
 * decides which conversation to hold on it.
 */
export class AcpProbe {
  ...
  #nextId = 1;
  /** Set once the CLI can no longer answer, so later calls fail instead of waiting forever. */
   ...
   * decision, and the failures that resolution raises — a CLI that is not installed, a bundled one
   * that cannot run — are classifications Ora acts on, which would be flattened into an
   * undifferentiated probe error here.
   */
  static attach(
    ...
    void child.exited.then(({ code, signal }) => {
      probe.#end(
        new AcpProbeError(
          `the probe CLI exited (code ${code}, signal ${signal}) before answering`,
        ),
      );
    });
    return probe;
  ...
  async #pumpStdout(): Promise<void> {
    try {
      ...
      this.#end(new AcpProbeError("the probe CLI closed its ACP stream"));
    } catch (error) {
      this.#end(
  ...
  /**
   * Drains stderr so a chatty CLI cannot grow an unread buffer for the probe's lifetime, and keeps
   * the diagnostics that explain a handshake which never completed.
   */
